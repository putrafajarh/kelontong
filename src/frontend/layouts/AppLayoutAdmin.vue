...
import { convertToKebabCase } from '@shared/utils/string'
...
const navigation = [
  ...
  { name: 'Users', href: '/user', icon: UsersIcon, current: route.path === '/user' },
  { name: 'Products', href: '/product', icon: ArchiveBoxIcon, current: route.path === '/product' },
]
...
useHead({
    bodyAttrs: {
      class: () => convertToKebabCase(route.meta.layout)
    }
})
</script>
      ...
      <div class="flex grow flex-col gap-y-5 overflow-y-auto bg-indigo-600 px-6 pb-4">
        ...
        <nav class="flex flex-1 flex-col">
          <ul role="list" class="flex flex-1 flex-col gap-y-7">
            ...
            <li class="mt-auto">
              <a href="#" @click.prevent="handleLogout" class="group -mx-2 flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6 text-indigo-200 hover:bg-indigo-700 hover:text-white">
                <ArrowRightOnRectangleIcon class="h-6 w-6 shrink-0 text-indigo-200 group-hover:text-white" aria-hidden="true" />
                Logout
              </a>
            </li>
          </ul>
              ...
              <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                <MenuItems class="absolute right-0 z-10 mt-2.5 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                  <MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
                    <a :href="item.href" @click.prevent="item.href === '/logout' && handleLogout()" :class="[active ? 'bg-gray-50' : '', 'block px-3 py-1 text-sm leading-6 text-gray-900']">{{ item.name }}</a>
                  </MenuItem>
                </MenuItems>
              </transition>
            </Menu>
